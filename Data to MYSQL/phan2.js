...
const MQTT_TOPICS = ['Data', 'WarningStatus'];
...
client.on('message', (topic, message) => {
    ...
    try {
        ...
        if (topic === 'Data') {
            const { Dust } = data;
            if (Dust !== undefined) {
                const sql = `INSERT INTO data (Dust, Time) VALUES (?, NOW())`;
                const values = [Dust];
                ...
                console.error('Invalid Data payload: Missing Dust field.');
            }
        } else if (topic === 'WarningStatus') {
            const { Device, Action } = data;
            if (Device && Action) {
                const query = 'INSERT INTO led (Device, Action) VALUES (?, ?)';
